...
setup.displaySavedImage = async function () {
    ...
    try {
        const base64Image = await setup.queryImageDB("portraitImage");
...
setup.displayRecentGeneratedImage = async function () {
    ...
    try {
        const base64Image = await setup.queryImageDB("dalleImage");
        setup.firstPortraitGen = true;
    ...
    ` : '';
        ...
        const sidebarHTML = `
            <div id="sidebar" class="twine-sidebar">
                ...
                `<img class="portraitImage portrait" src="" style="--gender-color: ${getGenderColor(State.variables.mc.gender)}; background-image: url('images/Layer Intros/l${SugarCube.State.variables.currentLayer}intro.png')">` :
...
window.setup = {
    ...
    infoGen(type) {
        ...
        listButton.textContent = type === "Relic" ? "Inventory" : "Curses";
        listButton.onclick = function () {
            SugarCube.Engine.play(listButton.textContent);
        };
...
setup.generateCharacterID = function () {
    ...
    const d8 = State.variables.mc.osex === "male"
        ? Math.floor(Math.random() * 5) * 2 + 1  // Odd number between 1 and 9
        : Math.floor(Math.random() * 5) * 2;     // Even number between 0 and 8
    
    // Digit 9: Fitness (-5 to 5 mapped to 0-9)
    let d9;
    switch(State.variables.mc.fit) {
        case -5: d9 = Math.floor(Math.random() * 2); break;         // 0-1
        case -2: d9 = Math.floor(Math.random() * 2) + 2; break;     // 2-3
        case 0:  d9 = Math.floor(Math.random() * 2) + 4; break;     // 4-5
        case 2:  d9 = Math.floor(Math.random() * 2) + 6; break;     // 6-7
        case 5:  d9 = Math.floor(Math.random() * 2) + 8; break;     // 8-9
        default: d9 = 0; // Default to 0 if fit value is unexpected
    }
